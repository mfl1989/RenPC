...
public class OrderSubmitRequestDTO {
        ...
        @Pattern(regexp = "^(self_erase_free|full_service_paid)$", message = "データ消去オプションの値が不正です")
        private String dataErasureOption;

        @NotBlank(message = "回収希望日を選択してください")
        @Pattern(regexp = "^\\d{4}-\\d{2}-\\d{2}$", message = "日付の形式が正しくありません")
        private String collectionDate;

        @NotBlank(message = "回収希望時間帯を選択してください")
        @Pattern(regexp = "^(unspecified|morning|t12_14|t14_16|t16_18|t18_21)$", message = "回収希望時間帯の値が不正です")
        ...
        @Pattern(regexp = "^[\\u30A1-\\u30F6\\u30FC\\u30FB\\u3000\\s]+$", message = "フリガナは全角カタカナで入力してください")
        ...
        @Pattern(regexp = "^[0-9+\\-()]{10,16}$", message = "電話番号の形式をご確認ください")
